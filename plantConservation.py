...
class PlantConservationDAO:
    ...
    def get_all(self):
        try:
            ...
            print(f"Error getting all conservations: {e}")

    def get_by_conservation_id(self, conservation_id):
        try:
            query = f"SELECT * FROM 养护 WHERE 任务编号 = ?"
            return self.conn.exec_sql_with_params(query, conservation_id)
        except Exception as e:
            print(f"Error getting by conservation id: {e}")
    ...
    def insert_conservation(self, conservation):
        try:
            ...
            print(f"Error inserting conservation: {e}")
    ...
    def update_conservation(self, conservation):
        try:
            ...
            print(f"Error updating conservation: {e}")
    ...
    def delete_conservation(self, conservation_id):
        try:
            ...
            print(f"Error deleting conservation: {e}")


class ConservationService:
    def __init__(self, conservation_dao):
        self.conservation_dao = conservation_dao
    ...
    def manage_menu(self):
        while True:
            ...
            try:
                ...
                if op == 1:
                    all_conservation = self.conservation_dao.get_all()
                    print("所有养护任务信息:")
                    ...
                    id = input("请输入养护任务编号: ")
                    conservations = self.conservation_dao.get_by_conservation_id(id)
                    ...
                    self.conservation_dao.insert_conservation(conservation)
                    pass
                elif op == 4:
                    ...
                    object = input("请输入养护任务对象: ")
                    conservation = PlantConservation(id, name, time, location, stuff, description, object)
                    self.conservation_dao.update_conservation(conservation)
                    pass
                elif op == 5:
                    id = input("请输入待删除的养护任务编号: ")
                    self.conservation_dao.delete_conservation(id)
                    pass
                elif op == 6:
                    print("退出植物养护管理系统。")
                    break
